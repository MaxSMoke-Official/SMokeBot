...
async function api (who, method, params) {
	return new Promise((res, err) => {
		...
		rq({"url": url, "method": "post", "form": params}, (e, r, b) => {
			return err(e), res(b)
		})
	})
}
...
async function getLongPollServer(callback) {
	...
	let longpoll = await api("group", method, {"group_id": group_id, "v" : 5.126})
	let bool = true
	setInterval(async () => {
		let event
		if (!bool) return;
		 bool = false
		event = await rq('get', {"url": `${longpoll.server}?act=a_check&key=${longpoll.key}&ts=${longpoll.ts}&wait=20` })
		if (event.updates) lp.ts = event.ts, bool = true
